import re
with open('partials/header.js', 'r', encoding='utf-8') as f:
    content = f.read()

base_logic = '''let basePath = '';
if (window.location.pathname.indexOf('/weight-loss-surgery/') !== -1) {
  basePath = '../';
}

'''
if 'let basePath' not in content:
...
with open('partials/header.js', 'w', encoding='utf-8') as f:
    f.write(content)
print('Done!')
